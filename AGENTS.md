...
edition/        # single source of truth for per-event content still hardcoded: Sponsors, tier company lists, FAQ, branding, actions.ts (action names + booth-to-action mapping) — the schedule/timetable moved to the DB (ScheduleEvent model, admin CRUD + drag-and-drop reorder board), don't add it back here
hooks/          # React hooks
...
- **Edition-specific content** still hardcoded (sponsors, tier company lists, booth-to-action mapping, FAQ, branding) is centralized under `src/edition/` — `edition/actions.ts` holds `actionNames` and the `getBoothActionName()` lookup that `savedStudentService.saveStudent()` calls instead of an inline `switch`. When editing that content for a new event, change `edition/`, not `config/` or `utils/`. The schedule/timetable is DB-backed instead (`ScheduleEvent` model), editable through the admin backoffice — don't add a static `edition/` file for it.
...
- **Where new code goes:**
  ...
  | Per-edition content still hardcoded (sponsors, FAQ, branding) | `edition/` |
