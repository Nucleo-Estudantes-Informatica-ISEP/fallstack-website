...
function patchRequest(body: unknown) {
  return new NextRequest("http://localhost/api/admin/schedule/order", {
    method: "PATCH",
...
test("PATCH rejects a malformed body with 400, without persisting anything", async () => {
  ...
  const res = await PATCH(
    patchRequest({ updates: [{ id: "not-a-uuid", day: 1, order: 0 }] }),
    { params: Promise.resolve({}) }
  );
...
test("PATCH commits the reorder for a valid admin request", async () => {
  ...
  const updates = [
    { id: "3fa85f64-5717-4562-b3fc-2c963f66afa6", day: 1, order: 0 },
  ];
  ...
  assert.equal(body.message, "Schedule order updated");
