...
test("limits one student without blocking another on same event Wi-Fi", async () => {
  createSignedUploadUrl.mockResolvedValue({
    data: { token: "signed-token", path: "distribution/cv/id.pdf" },
    error: null,
  });
...
test("returns an upload ticket without receiving CV bytes", async () => {
  createSignedUploadUrl.mockResolvedValue({
    data: { token: "signed-token", path: "distribution/cv/id.pdf" },
    error: null,
  });
  ...
  expect(await res.json()).toEqual({
    ...
    path: expect.stringMatching(/^distribution\/cv\/.+\.pdf$/),
    token: "signed-token",
  });
});
