...
const schedulePositionConflict = () =>
  new HttpError(
    "Outro administrador já utilizou esta posição. Atualize e tente novamente.",
    409
  );

async function handleSchedulePositionConflict<T>(operation: () => Promise<T>) {
  try {
    return await operation();
  } catch (error) {
    if (isUniqueSchedulePositionError(error)) throw schedulePositionConflict();
    throw error;
  }
}
...
export async function createScheduleEventForAdmin(input: {
  ...
  // permits swaps inside one transaction while rejecting a stale concurrent plan.
  return handleSchedulePositionConflict(() =>
